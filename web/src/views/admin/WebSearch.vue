  ...
  <div>
    <div class="section-header">
      <div class="section-title">
        <span class="section-eyebrow">07 / WEB SEARCH · {{ $adminText('联网') }}</span>
        <h2>{{ $adminText('联网功能配置') }}</h2>
      </div>
    </div>

    <div class="admin-card" v-loading="wsLoading">
      <h3 style="font-size:16px;font-weight:600;margin-bottom:16px;color:var(--ink);">{{ $adminText('联网搜索（Tavily）') }}</h3>

      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
        <span style="font-size:13px;color:var(--ink-3);width:150px;">{{ $adminText('联网搜索总开关') }}</span>
        <el-switch v-model="wsEnabled" :disabled="!wsHasKey && !wsApiKey" />
        <span style="font-size:13px;color:var(--ink-3);">{{ $adminText(wsEnabled ? '已开启：聊天输入框将展示“联网”按钮' : '已关闭：聊天页面不显示联网按钮') }}</span>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
        ...
        <el-input
          v-model="wsApiKey"
          :placeholder="wsHasKey ? wsApiKeyMasked + ' (' + $adminText('已配置，留空表示不修改') + ')' : $adminText('请输入 tvly- 开头的 API Key')"
          clearable
          style="width:360px;"
        />
        <el-button @click="handleTestWebSearch" :loading="wsTesting" :disabled="!wsHasKey && !wsApiKey">{{ $adminText('测试连接') }}</el-button>
        <el-button type="primary" @click="handleSaveWebSearch" :loading="wsSaving">{{ $adminText('保存') }}</el-button>
      </div>
      ...
      <div style="font-size:13px;color:var(--ink-3);line-height:1.8;background:var(--paper-2);padding:12px 16px;border-radius:8px;border:1px solid var(--line);">
        {{ $adminText('开启后，用户在聊天输入框可开启“联网”，回答前先通过 Tavily 检索实时网络信息并作为参考资料提供给模型。') }}
        <b style="color:#f56c6c">{{ $adminText('必须先配置 API Key 才能开启总开关；关闭总开关后聊天页面的联网按钮会隐藏。') }}</b>
        API Key {{ $adminText('可在') }} <a href="https://tavily.com" target="_blank" style="color:var(--accent);">tavily.com</a> {{ $adminText('免费申请，保存后仅显示掩码。') }}
      </div>
    </div>
  </div>
</template>
...
async function handleSaveWebSearch() {
  if (wsEnabled.value && !wsHasKey.value && !wsApiKey.value.trim()) {
    ElMessage.warning(adminText('请先配置 Tavily API Key 再开启联网搜索'))
    return
  }
  wsSaving.value = true
  try {
...
async function handleTestWebSearch() {
  ...
  try {
    ...
    if (res?.success) {
      ...
      ElMessage.error(adminApiText(res?.message, '连接失败'))
    }
  } finally {
