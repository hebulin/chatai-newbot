  ...
  <div>
    <div class="section-header">
      <div class="section-title">
        <span class="section-eyebrow">02 / MODELS · {{ $adminText('模型') }}</span>
        <h2>{{ $adminText('模型管理') }}</h2>
      </div>
      <div style="display:flex;gap:8px;">
        ...
        <el-button type="primary" @click="showAddModel">
          <el-icon><Plus /></el-icon> {{ $adminText('添加模型') }}
        </el-button>
      </div>
    </div>
    ...
    <div class="admin-card">
      ...
      <div class="filter-bar" :class="{ 'filter-collapsed': !filterExpanded }">
        <el-input v-model="filters.name" :placeholder="$adminText('名称模糊查询')" clearable style="width:180px" @input="applyFilter" />
        <el-select v-model="filters.providerId" :placeholder="$adminText('全部厂商')" clearable style="width:150px" @change="applyFilter">
          <el-option v-for="opt in providerFilterOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
        </el-select>
        <el-select v-model="filters.modelId" class="filter-extra" :placeholder="$adminText('全部模型')" clearable filterable style="width:180px" @change="applyFilter">
          <el-option v-for="opt in modelIdFilterOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
        </el-select>
        <el-select v-model="filters.thinking" class="filter-extra" :placeholder="$adminText('思考')" clearable style="width:110px" @change="applyFilter">
          <el-option :label="$adminText('支持')" value="1" />
          <el-option :label="$adminText('不支持')" value="0" />
        </el-select>
        <el-select v-model="filters.multimodal" class="filter-extra" :placeholder="$adminText('多模态')" clearable style="width:110px" @change="applyFilter">
          <el-option :label="$adminText('支持')" value="1" />
          <el-option :label="$adminText('不支持')" value="0" />
        </el-select>
        <el-select v-model="filters.enabled" class="filter-extra" :placeholder="$adminText('状态')" clearable style="width:110px" @change="applyFilter">
          <el-option :label="$adminText('已启用')" value="1" />
          <el-option :label="$adminText('已禁用')" value="0" />
        </el-select>
        <el-select v-model="filters.visible" class="filter-extra" :placeholder="$adminText('可见性')" clearable style="width:120px" @change="applyFilter">
          <el-option :label="$adminText('所有人')" value="1" />
          <el-option :label="$adminText('仅管理员')" value="0" />
        </el-select>
        <el-button @click="resetFilter">{{ $adminText('重置') }}</el-button>
        <el-button class="filter-toggle-btn" @click="filterExpanded = !filterExpanded">
          {{ $adminText(filterExpanded ? '收起' : '更多筛选') }}
      ...
      <el-table ref="tableRef" :data="pagedModels" v-loading="loading" stripe border style="width:100%"
        ...
        <el-table-column :label="$adminText('名称')" :width="colW.name">
          <template #default="{ row }">
            <div class="model-name-cell">
              ...
              <span v-if="row.id === defaultModelId" class="default-badge">{{ $adminText('默认') }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="providerName" :label="$adminText('厂商')" :width="colW.provider" show-overflow-tooltip />
        <el-table-column prop="apiUrl" :label="$adminText('API 地址')" min-width="220" show-overflow-tooltip />
        <el-table-column :label="$adminText('模型')" :width="colW.modelId" show-overflow-tooltip>
          <template #default="{ row }">
            <span class="model-id-text">{{ row.modelId }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('思考')" :width="colW.thinking" align="center">
          <template #default="{ row }">
            <span v-if="row.supportsThinking" class="think-badge">{{ $adminText('支持') }}</span>
            <span v-else style="color:var(--ink-4)">{{ $adminText('不支持') }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('多模态')" :width="colW.multimodal" align="center">
          <template #default="{ row }">
            <span v-if="row.supportsMultimodal" class="mm-badge">{{ $adminText('支持') }}</span>
            <span v-else style="color:var(--ink-4)">{{ $adminText('不支持') }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('状态')" :width="colW.status" align="center">
          <template #default="{ row }">
            ...
            >{{ $adminText(row.enabled ? '已启用' : '已禁用') }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('可见性')" :width="colW.visible" align="center">
          <template #default="{ row }">
            <span class="status-badge" :class="row.visibleToAll ? 'vis-all' : 'vis-admin'">
              {{ $adminText(row.visibleToAll ? '所有人' : '仅管理员') }}
            </span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('延迟')" :width="colW.latency" align="center">
          <template #default="{ row }">
            <span v-if="row.testLatencyMs != null" class="metric-text" :class="latencyClass(row.testLatencyMs)" :title="row.testedAt ? $adminText('测试于 {time}', { time: row.testedAt }) : ''">{{ row.testLatencyMs }} ms</span>
            <span v-else class="metric-empty" :title="$adminText('点击右侧测试连接后更新')">-</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('速度')" :width="colW.speed" align="center">
          <template #default="{ row }">
            <span v-if="row.testSpeed != null" class="metric-text" :title="row.testedAt ? $adminText('测试于 {time}', { time: row.testedAt }) : ''">{{ row.testSpeed }} tk/s</span>
            <span v-else class="metric-empty" :title="$adminText('点击右侧测试连接后更新')">-</span>
          </template>
        </el-table-column>
        <el-table-column :label="$adminText('操作')" width="160" align="center" fixed="right">
          <template #default="{ row }">
            <el-button-group>
              ...
              <el-button size="small" text :title="$adminText('测试连接')" :disabled="testingIds.has(row.id)" @click="handleTest(row)">
    ...
    <!-- 编辑模型弹窗 -->
    <el-dialog v-model="editVisible" :title="$adminText('编辑模型 - {name}', { name: editForm.displayName || editForm.modelId })" width="520px" destroy-on-close>
      <el-form label-width="90px">
        ...
        <el-form-item :label="$adminText('协议格式')">
        ...
        <el-form-item label="API Key">
          <el-input v-model="editForm.apiKey" :placeholder="$adminText('不修改则留空')" />
        </el-form-item>
        <el-form-item :label="$adminText('模型名称')">
          <el-input v-model="editForm.displayName" />
        </el-form-item>
        <el-form-item :label="$adminText('模型ID')">
          <el-input v-model="editForm.modelId" disabled />
        </el-form-item>
        <div class="form-section-divider"></div>
        <el-form-item :label="$adminText('输入价格')">
          <el-input-number v-model="editForm.inputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" />
          <span class="pricing-hint">{{ $adminText('人民币元 / 百万 Token') }}</span>
        </el-form-item>
        <el-form-item :label="$adminText('输出价格')">
          <el-input-number v-model="editForm.outputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" />
        </el-form-item>
        <el-form-item :label="$adminText('缓存价格')">
          <el-input-number v-model="editForm.cachedPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" />
        </el-form-item>
        <el-form-item :label="$adminText('思考价格')">
          <el-input-number v-model="editForm.reasoningPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" />
          <span class="pricing-hint">{{ $adminText('填 0 时按输出价格计算') }}</span>
        </el-form-item>
        <div class="form-section-divider"></div>
        <el-form-item :label="$adminText('状态')">
          <el-switch v-model="editForm.enabled" />
        </el-form-item>
        <el-form-item :label="$adminText('可见性')">
          <el-switch v-model="editForm.visibleToAll" :active-text="$adminText('所有人')" :inactive-text="$adminText('仅管理员')" />
        </el-form-item>
        <el-form-item :label="$adminText('思考模式')">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveModel" :loading="submitting">{{ $adminText('保存') }}</el-button>
      </template>
    </el-dialog>

    <!-- 添加模型弹窗 -->
    <el-dialog v-model="addVisible" :title="$adminText('添加模型')" width="540px" destroy-on-close>
      <el-form label-width="90px">
        <el-form-item :label="$adminText('厂商')">
          <el-select v-model="addForm.providerId" filterable style="width:100%" @change="onProviderChange">
            <el-option v-for="p in allProviders" :key="p.id" :label="p.name + (p.type === 'custom' ? $adminText('（自定义）') : '')" :value="p.id" />
            <el-option :label="$adminText('新建自定义厂商...')" value="__custom__" />
          </el-select>
        </el-form-item>
        <!-- 自定义厂商名称 -->
        <el-form-item v-if="addForm.providerId === '__custom__'" :label="$adminText('厂商名称')">
          <el-input v-model="addForm.customProviderName" :placeholder="$adminText('如：OpenAI')" />
        </el-form-item>
        <!-- 已有厂商模型选择 -->
        <el-form-item v-if="addForm.providerId !== '__custom__' && addForm.providerId" :label="$adminText('模型')">
          <el-select v-model="addForm.modelSelect" filterable style="width:100%" @change="onModelSelectChange">
            <el-option v-for="pm in currentProviderModels" :key="pm.id" :label="pm.name + (pm.supportsThinking ? ' (' + $adminText('思考') + ')' : '') + (pm.supportsMultimodal ? ' (' + $adminText('多模态') + ')' : '') + (isModelAdded(pm.id) ? $adminText('（已接入）') : '')" :value="pm.id" />
            <el-option :label="$adminText('自定义模型...')" value="__custom__" />
          </el-select>
        </el-form-item>
        ...
        <!-- 协议格式 -->
        <el-form-item v-if="addForm.providerId === '__custom__'" :label="$adminText('协议格式')">
          <el-select v-model="addForm.protocol" style="width:100%">
            <el-option :label="$adminText('OpenAI 兼容')" value="openai" />
            <el-option label="Anthropic" value="anthropic" />
          </el-select>
        </el-form-item>
        <el-form-item v-else-if="addForm.providerId" :label="$adminText('协议格式')">
        ...
        <template v-if="isPresetMode">
          ...
          <el-form-item :label="$adminText('输入价格')"><el-input-number v-model="addForm.inputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <el-form-item :label="$adminText('输出价格')"><el-input-number v-model="addForm.outputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <el-form-item :label="$adminText('缓存价格')"><el-input-number v-model="addForm.cachedPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <el-form-item :label="$adminText('思考价格')"><el-input-number v-model="addForm.reasoningPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <div class="form-section-divider"></div>
          <el-form-item :label="$adminText('可见性')">
            <el-switch v-model="addForm.visibleToAll" :active-text="$adminText('所有人')" :inactive-text="$adminText('仅管理员')" />
          </el-form-item>
          <el-form-item :label="$adminText('能力')">
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <span :style="{ fontSize:'13px', padding:'4px 12px', borderRadius:'6px', background: selectedPresetModel?.supportsThinking ? 'rgba(16,185,129,.15)' : 'rgba(100,116,139,.1)', color: selectedPresetModel?.supportsThinking ? '#10b981' : '#64748b' }">
                {{ $adminText('思考模式：{value}', { value: $adminText(selectedPresetModel?.supportsThinking ? '支持' : '不支持') }) }}
              </span>
              <span :style="{ fontSize:'13px', padding:'4px 12px', borderRadius:'6px', background: selectedPresetModel?.supportsMultimodal ? 'rgba(99,102,241,.15)' : 'rgba(100,116,139,.1)', color: selectedPresetModel?.supportsMultimodal ? '#818cf8' : '#64748b' }">
                {{ $adminText('多模态：{value}', { value: $adminText(selectedPresetModel?.supportsMultimodal ? '支持' : '不支持') }) }}
              </span>
            </div>
          </el-form-item>
        </template>
        ...
        <template v-else>
          <el-form-item :label="$adminText('API 地址')">
            <el-input v-model="addForm.apiUrl" :placeholder="$adminText('API地址')" />
          </el-form-item>
          <el-form-item label="API Key">
            <el-input v-model="addForm.apiKey" placeholder="sk-..." />
          </el-form-item>
          <el-form-item :label="$adminText('模型名称')">
            <el-input v-model="addForm.displayName" :placeholder="$adminText('可选，默认使用模型ID')" />
          </el-form-item>
          <el-form-item :label="$adminText('模型ID')">
            <el-input v-model="addForm.modelId" :placeholder="$adminText('如 gpt-4o')" />
          </el-form-item>
          <el-form-item :label="$adminText('输入价格')"><el-input-number v-model="addForm.inputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <el-form-item :label="$adminText('输出价格')"><el-input-number v-model="addForm.outputPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
          <el-form-item :label="$adminText('缓存价格')"><el-input-number v-model="addForm.cachedPriceCny" :min="0" :precision="6" :step="0.1" style="width:100%" /></el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="submitAddModel" :loading="submitting">{{ $adminText('添加') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const colW = computed(() => {
  ...
  return {
    ...
    name: autoColWidth(
      list.map(m => (m.displayName || m.modelId || '') + (m.id === defaultModelId.value ? ` ${adminText('默认')}` : '')),
      { header: adminText('名称'), extra: 40, min: 120 }
    ),
    provider: autoColWidth(list.map(m => m.providerName), { header: adminText('厂商') }),
    // 模型ID列以等宽字体渲染，按 mono 测宽
    modelId: autoColWidth(list.map(m => m.modelId), { header: adminText('模型'), min: 100, mono: true }),
    thinking: autoColWidth(['支持', '不支持'].map(adminText), { header: adminText('思考'), extra: 16 }),
    multimodal: autoColWidth(['支持', '不支持'].map(adminText), { header: adminText('多模态'), extra: 16 }),
    status: autoColWidth(['已启用', '已禁用'].map(adminText), { header: adminText('状态'), extra: 24 }),
    visible: autoColWidth(['所有人', '仅管理员'].map(adminText), { header: adminText('可见性'), extra: 24 }),
    latency: autoColWidth(list.map(m => m.testLatencyMs != null ? `${m.testLatencyMs} ms` : '-'), { header: adminText('延迟'), mono: true }),
    speed: autoColWidth(list.map(m => m.testSpeed != null ? `${m.testSpeed} tk/s` : '-'), { header: adminText('速度'), mono: true })
  }
})
...
async function toggleModel(row) {
  ...
  if (res?.success) {
    ElMessage.success(adminText(!row.enabled ? '已启用' : '已禁用'))
    await loadData()
  }
}
...
async function handleDefault(row) {
  if (row.id === defaultModelId.value) {
    ...
    if (res?.success) { ElMessage.success(adminApiText(res.message, '已取消默认模型')); await loadData() }
  } else {
    const res = await setDefaultModel(row.id)
    if (res?.success) { ElMessage.success(adminApiText(res.message, '已设为默认模型')); await loadData() }
  }
}
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(adminText('确定删除模型 "{name}" 吗？', { name: row.displayName || row.modelId }), adminText('确认删除'), { type: 'warning' })
    const res = await deleteModel(row.id)
    if (res?.success) { ElMessage.success(adminText('已删除')); await loadData() }
    else ElMessage.error(adminApiText(res?.message, '删除失败'))
  } catch { /* cancelled */ }
}
...
async function handleDeleteSelected() {
  ...
  try {
    await ElMessageBox.confirm(adminText('确定删除选中的 {count} 个模型吗？', { count: rows.length }), adminText('确认批量删除'), { type: 'warning' })
    const res = await batchDeleteModels(rows.map(r => r.id))
    if (res?.success) {
      ElMessage.success(adminText('已删除 {count} 个模型', { count: res.deleted ?? rows.length }))
...
async function handleTest(row) {
  ...
  try {
    ...
    if (res?.success) {
      ElMessage.success(adminApiText(res.message, '连接成功'))
    } else {
      ElMessage.error({ message: adminText('连接失败：{message}', { message: adminApiText(res?.message, '未知错误') }), duration: 6000 })
    }
...
async function submitAddModel() {
  ...
  if (isCustomProvider) {
    if (!addForm.value.customProviderName.trim()) { ElMessage.warning(adminText('请输入厂商名称')); return }
    if (!addForm.value.modelId.trim()) { ElMessage.warning(adminText('请输入模型ID')); return }
    if (!addForm.value.apiKey.trim()) { ElMessage.warning(adminText('请输入 API Key')); return }
    payload = {
    ...
    if (!pm) { ElMessage.warning(adminText('请选择模型')); return }
    payload = {
