...
### 管理后台
- **上下文与输出大小**：在“系统设置 → 上下文大小”和“系统设置 → 输出大小”分别配置，均支持全局默认与单个模型覆盖。上下文默认 32000 Token，为输入、历史及输出的总容量；输出默认 16384 Token，包含思考。上下文必须为正整数，输出可设 0 表示不额外限制。模型未配置时继承对应全局值，点击“删除”清除覆盖；留空输入不会自动保存默认值。两项配置互不覆盖，保存后新请求生效。
- **配置存储与迁移**：上下文全局值为 `t_setting.chat_context_window`，模型覆盖为 `t_model_config.context_window`（旧值 0/NULL 继承全局）；启动自动补列，保留旧库已有正容量，不重置。输出全局值为 `t_setting.chat_max_output_tokens`，模型覆盖为可空的 `max_output_tokens`（NULL 继承、0 不限）。既有输出继承迁移保持一次性执行。无需手动 SQL，部署后重启即可完成迁移。
- **请求预算**：先按总上下文容量预留输出并裁剪早期完整对话轮次，优先保留当前问题；当前问题本身超限时明确报错。最终组装后计入附件、搜索与摘要，两种协议均使用 `min(输出上限, 上下文剩余容量)`，输出不限时只受配置的上下文剩余容量约束，不再使用固定 32000 估算输出。容量仍需按厂商实际能力配置，估算不能替代厂商 Token 计数。达到上限保留正文、思考、用量与原因；恢复草稿也保留原因。
- **长内容保存**：正文、思考及回答版本以完整 JSON 存入 SQLite `t_chat_session.messages TEXT`；只有列表标题/预览会缩略。回归测试已覆盖百万字符正文及思考、版本历史的 HTTP 保存和数据库读回。此结论不表示存储无限：极端大小仍受 JSON 解析器、内存、磁盘及 SQLite 单值上限约束；浏览器本地草稿容量不足时降级，不影响正常服务端保存。
...
- **流式完成判定**：HTTP 正常关闭不等于模型完成；缺少有效 `finish_reason` 或 `[DONE]` 时，保留部分正文、思考、用量和恢复草稿，并显示中断原因及继续生成入口。损坏/截断的结束事件不能作为完成信号。
